...
val creditSubDf = creditDf.filter("repaymentAbility == 1")
...
val paramGrid = new ParamGridBuilder().
        addGrid(gm.k, Array(2, 3, 4, 5, 6, 7, 8, 9, 10)).
...
predictions2.
       select("duration", "amount", "instRate", "regiPeriod",
              "age", "existCredit", "dependents").
       where("prediction = 0").describe().show

predictions2.
       select("duration", "amount", "instRate", "regiPeriod",
              "age", "existCredit", "dependents").
       where("prediction = 1").describe().show